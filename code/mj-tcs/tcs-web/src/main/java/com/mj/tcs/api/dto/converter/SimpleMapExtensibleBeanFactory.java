...
public class SimpleMapExtensibleBeanFactory implements ExtensibleBeanFactory {
    ...
    public void registerDto(final String key, final String className) throws TCSServerRuntimeException {
        try {
            classes.put(key, Class.forName(className));
        } catch (ClassNotFoundException e) {
            throw new TCSServerRuntimeException(e);
        }
    }

    public void registerEntity(final String key, final String className, final String representative) throws TCSServerRuntimeException {
        try {
            classes.put(key, Class.forName(className));
